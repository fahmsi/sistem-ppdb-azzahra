...
test('parent can add two children and both belong to the same user', function () {
    ...
    $this->actingAs($parent)
        ->post(route('parent.siswa.store'), validMultiChildSiswaPayload('Alya'))
        ->assertRedirect();

    $this->actingAs($parent)
        ->post(route('parent.siswa.store'), validMultiChildSiswaPayload('Bima'))
        ->assertRedirect();
...
test('parent cannot access another parents child workflow', function () {
    $parent = User::factory()->create(['role' => 'parent']);
    $otherParent = User::factory()->create(['role' => 'parent']);
    $otherChild = Siswa::factory()->for($otherParent)->create();
    $period = openMultiChildPeriod();
...
test('two children can register in the same period while each child keeps one active registration', function () {
    $parent = User::factory()->create(['role' => 'parent']);
    ...
    $otherPeriod = openMultiChildPeriod(['gelombang' => 'Gelombang Lain']);
    $this->actingAs($parent)
...
test('status and payment remain tied to the selected child registration', function () {
    Storage::fake('local');
...
test('nested child urls reject mismatched registration and payment details', function () {
    Storage::fake('local');
...
test('legacy parent workflow urls redirect to child selection', function (string $url) {
    $parent = User::factory()->create(['role' => 'parent']);
...
test('dashboard renders with no children and with multiple children', function () {
    ...
    $this->actingAs($parent)
        ...
        ->assertOk()
        ->assertSee('Mulai pendaftaran anak')
...
test('account deletion is rejected when any child has registration history', function () {
    ...
    PendaftaranDetail::create([
        ...
        'pendaftaran_id' => openMultiChildPeriod()->id,
...
function openMultiChildPeriod(array $overrides = []): Pendaftaran
{
...
function validMultiChildSiswaPayload(string $name): array
{
    return [
        'nama' => "{$name} Azzahra",
        'nama_panggilan' => $name,
